...
### 夜通しつけっぱなしだった日は通知文を差し替える

1の「朝の使用確認」には落とし穴があります。**対象家電が夜通しつけっぱなしだと、朝の初回検知が「本人が起きた」ことを意味しません。** `NOTIFY_FROM_HOUR` の最初の観測で必ずしきい値を超えるためです。

しかもこれは誤報として現れません。正常な通知として毎日届くので、家族は気づけません。

そこで**当日0:00〜`NOTIFY_FROM_HOUR` の記録がすべてしきい値以上**だった場合は、通知文を差し替えます。

```
🌙 今朝 7:20 の時点でテレビがついていました（昨夜から消されていません）。
消さずに寝た可能性がありますが、念のため様子を確認してください。
※この日は起床の確認ができていません。
```

追加通知にせず**同じ1通の枠で文面を差し替える**のは、「使用を確認しました」と「実は夜通しついていた」が別々に届くと家族が混乱するためです。差し替えなら「この日は起床の確認になっていない」が必ず伝わります。

この検知は**「テレビをつけたまま倒れた」ケースの一部も拾えます。** 朝つけた後に倒れた場合、その夜も朝までつけっぱなしになるため、翌朝の判定で異常として現れます。

なお、ログの欠測で誤判定しないよう、深夜帯の記録が期待件数の半分未満のときは判定しません（回線断そのものは3のシステム異常が拾います）。

> この通知が**毎日届くようになったら、その家電は見守り対象として不適格**です。つけっぱなしが常態なら「消した／つけた」がシグナルにならないため、対象家電を選び直してください。

これは**本運用時**の通知です。セットアップ中は次の3段階で挙動が変わります。

| 段階 | フラグ | 通知 |
|---|---|---|
| 観察期間 | `OBSERVATION_MODE=true` | なし（ログ収集のみ） |
| 試作運用 | `TRIAL_MODE=true` | 対象家電のON/OFFを都度通知 |
| 本運用 | どちらもなし | 上表の4種類 |
...
8. **試作運用（数日〜1週間）**: `TRIAL_MODE=true` にしてON/OFFを都度通知し、生活パターンを家族が確認する（下記）
9. **本運用開始**: 両フラグを外し、`setupTriggers` を実行
...
**STEP 4**: 読み取った値をスクリプトプロパティに登録します。続いて試作運用に移ります。

### 試作運用 — ON/OFFを通知して生活パターンを確認する

観察期間でしきい値が決まっても、**それだけでは本運用の判定が成立するか分かりません。** 対象家電が夜通しつけっぱなしになっていると、朝の初回検知が「本人が起きた」ことを意味しなくなるためです。

そこで本運用の前に、`TRIAL_MODE=true` にして**ON/OFFの変化を都度通知する**段階を挟みます。

```
🔌 07:20 テレビがONになりました（60W）
⚫ 12:00 テレビがOFFになりました
```
...
| 見ること | 判断 |
|---|---|
| **OFF通知が毎日来るか** | 来なければ**つけっぱなし**。朝の初回検知が「起きた」を意味しないため、本運用の判定方式を見直す必要があります |
| ON通知の時刻の散らばり | `NOTIFY_FROM_HOUR` / `NOTIFY_TO_HOUR` の妥当性を確認できます |
| ON通知に載るW数 | `POWER_THRESHOLD` が適切かを確認できます |

家族が「どういう通知が届くか」を体感できる段階でもあります。

> ⚠️ **通知量が増えます。** ON/OFFで1日2〜6通になるため、LINEの無料枠（月200通）を数週間で使い切ります。**数日〜1週間で本運用に移行してください。** 試作運用中は朝の使用確認通知と見守りアラートは出ません。

`setupTrialTriggers` を実行すると `collectPower`（10分ごと）だけが登録されます。

### 本運用開始

`OBSERVATION_MODE` と `TRIAL_MODE` の**両方を削除（または `false` に）** してから `setupTriggers` を実行します。どちらかが残っていると実行を拒否するので、外し忘れには気づけます。

### 設定値は生活スタイルによって変わり続けます

`POWER_THRESHOLD` / `NOTIFY_FROM_HOUR` / `NOTIFY_TO_HOUR` と対象家電の選定は、**その人のその時点の生活に合わせた値**です。一度決めたら終わりではありません。

次のような変化があったときは、観察期間からやり直すことを想定しています。

- **季節の変わり目** — 起床時刻が動く。冬に起きるのが遅くなれば `NOTIFY_TO_HOUR` を後ろに倒す必要があります
- **生活リズムの変化** — 入院・退院、同居家族の増減、デイサービスの利用開始など
- **家電の買い替え** — 消費電力が変わればしきい値も変わります
- **習慣の変化** — テレビを見なくなった、つけっぱなしで寝るようになった、など。この場合は対象家電そのものを選び直します

`OBSERVATION_MODE=true` に戻して `setupObservationTriggers` を実行すれば、通知を止めたままログの取り直しができます。
...
| `TRIAL_MODE` | | `true` の間はON/OFFの変化を都度通知します。試作運用中に設定します（`OBSERVATION_MODE` が優先されます） |
| `ALLOW_SAMPLE_NOTIFICATIONS` | | `true` のときだけ `sendSampleNotifications` が動きます（スクリーンショット用の誤爆防止） |

`POWER_THRESHOLD` を必須にしているのは意図的です。デフォルト値があると「決めなくても動く」と誤読され、本人の生活に合っていないしきい値のまま静かに動き続けてしまいます。未設定ならジョブは明示的にエラーで止まります。

次の2つはスクリプトが自動で書き込むプロパティです。手で設定する必要はありません。

| プロパティ名 | 内容 |
|---|---|
| `LAST_ON_NOTIFIED_DATE` | 朝の使用確認通知を送った日付（`yyyy-MM-dd`）。同じ日に何度も通知しないための記録 |
| `LAST_POWER_STATE` | 試作運用モードで直前に観測したON/OFF状態。変化の検出に使う |
| `LAST_ERROR_NOTIFIED_<関数名>` | ジョブごとのエラー通知時刻。同じエラーの連投を防ぐための記録 |

エラー通知は**同じジョブにつき60分に1通まで**に間引いています。`collectPower` は10分ごとに動くため、SwitchBot APIの障害やプラグのオフラインが続くと1日144通が飛び、LINEの無料枠（月200通）を1日半で使い切って**本来のアラートが送れなくなる**からです。実行ログには毎回残すので、障害の全期間は追えます。
...
| 実行する関数 | タイミング | 必要なフラグ | 登録されるトリガー |
|---|---|---|---|
| `setupObservationTriggers` | 観察期間の開始時 | `OBSERVATION_MODE=true` | `collectPower`（10分ごと）のみ |
| `setupTrialTriggers` | 試作運用の開始時 | `TRIAL_MODE=true` | `collectPower`（10分ごと）のみ |
| `setupTriggers` | 本運用の開始時 | 両フラグなし | `collectPower`（10分ごと）／`morningCheck`（毎日 `NOTIFY_TO_HOUR` 時台）／`weeklySummary`（日曜20時台） |

確認・やり直しには `showTriggers`（一覧表示）と `deleteAllTriggers`（全削除）を使います。何度実行しても重複登録されません（既存トリガーを削除してから登録します）。

## サンプル通知の送信（スクリーンショット用）

`sendSampleNotifications` を手動実行すると、全7種類の通知をサンプル値で送信します。ドキュメントや記事に載せるスクリーンショットを撮るためのものです。
...
**文面は本番のジョブと同じ生成関数（`jobs.gs` の `build*Message_`）を使っています。** スクリーンショットが実際に届く通知とずれないようにするためです。通知文を変更しても、サンプル側の修正は不要です。

> ⚠️ **家族グループに送る前に一声かけてください。** 「見守りアラート」など不安にさせる文面が含まれます。誤爆防止として `ALLOW_SAMPLE_NOTIFICATIONS=true` を設定しないと実行できないようにしていますが、それでも送信先は本番の `LINE_GROUP_ID` です。自分だけのグループへ一時的に向けてから実行するのが安全です。撮影後は `ALLOW_SAMPLE_NOTIFICATIONS` を削除してください。

この方式にしている理由は3つあります。

- **`morningCheck` の時刻を `NOTIFY_TO_HOUR` から自動で決める** — この2つがずれると「朝の通知も来ないがアラートも出ない」空白時間が生じます。手で揃えさせるのではなくコードで保証しています
- **フラグの付け外し忘れを防ぐ** — 各関数が対応するフラグの状態を検査し、合っていなければ実行を拒否します
- **設定不備をセットアップ時点で発見する** — いずれも先に `getConfig_()` を呼ぶため、プロパティの不足は「10分ごとのエラー通知」ではなく実行時のエラーとして分かります
