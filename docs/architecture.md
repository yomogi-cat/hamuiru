...
flowchart TD
    subgraph home["祖母宅"]
        pot["電気ポット"] --- plug["SwitchBot プラグミニ<br>(電力計測)"]
...
| 関数 | トリガー | 役割 |
|---|---|---|
| `collectPower` | 30分ごと | プラグの電力値を取得し `log` シートへ追記 |
| `morningCheck` | 毎日 10〜11時 | 当日0:00以降のログで生存判定。異常時のみLINE通知 |
...
## シーケンス: 朝の生存判定（morningCheck・毎朝10時）
...
sequenceDiagram
    ...
    else power_w ≥ しきい値 の行が0件
        G->>N: 🔔 見守りアラート<br>(①電話 → ②訪問 の対応手順)
    else 使用形跡あり
...
- **設定不備**: `validateConfig_()` が不足しているスクリプトプロパティ名を列挙して例外を投げるため、セットアップ漏れが実行ログから即座に分かる。
