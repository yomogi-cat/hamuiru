...
| 1' | つけっぱなし通知 | 1の信頼性の担保 | 1と同じタイミング。深夜帯がすべてしきい値以上なら1の文面を差し替える |
...
sequenceDiagram
    ...
    alt weight ≥ POWER_THRESHOLD かつ<br>NOTIFY_FROM_HOUR ≤ 時 < NOTIFY_TO_HOUR かつ<br>当日未通知 かつ OBSERVATION_MODE=false
        G->>L: 当日0:00〜NOTIFY_FROM_HOUR の行を走査
        alt 深夜帯がすべて POWER_THRESHOLD 以上
            G->>N: 🌙 今朝 HH:mm の時点で{家電}がついていました<br>※この日は起床の確認ができていません
        else 深夜帯に消された形跡がある
...
朝の初回検知は「本人が起きた」ことを意味する前提で成り立っている。しかし対象家電が夜通しつけっぱなしだと、`NOTIFY_FROM_HOUR` の最初の観測で必ずしきい値を超えるため、この前提が崩れる。しかも誤報としては現れず、正常な通知として毎日届くので家族は気づけない。

そのため `wasOnAllNight_` で**当日0:00〜`NOTIFY_FROM_HOUR` の記録がすべてしきい値以上か**を判定し、該当する日は通知文を差し替える。

**追加通知にしないのは意図的である。** 「使用を確認しました」と「実は夜通しついていた」が別々に届くと家族が混乱するため、同じ1通の枠で文面を差し替え、「この日は起床の確認になっていない」を必ず伝える。

副次的な効果として、**「家電をつけたまま倒れた」ケースの一部を拾える。** 朝つけた後に倒れた場合、その夜も朝までつけっぱなしになるため、翌朝の判定で異常として現れる。従来は「使用形跡あり」として正常扱いされ、永久に検知できなかった。

ログの欠測で誤判定しないよう、深夜帯の記録が期待件数（`NOTIFY_FROM_HOUR * 60 / COLLECT_INTERVAL_MINUTES_`）の半分未満のときは判定しない。回線断そのものは `morningCheck` のシステム異常通知が拾う。

この通知が毎日届く場合、その家電は見守り対象として不適格である（つけっぱなしが常態なら「消した／つけた」がシグナルにならない）。対象家電を選び直す判断材料になる。

なお `COLLECT_INTERVAL_MINUTES_` は `jobs.gs` で定義している。`setup.gs` のトリガー登録と、この期待件数の算出の両方で使うためである。
